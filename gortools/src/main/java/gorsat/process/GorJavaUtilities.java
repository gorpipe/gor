...
public class GorJavaUtilities {
    ...
    public static List<String> getUsedFiles(String commandToExecute, GorSession session) {
     ...
     * @return
     */
    private static Optional<Tuple2<String,Boolean>> resolveForkPathParent(String res) {
        var i = res.indexOf("#{");
        if(i != -1) {
    ...
    private static Optional<Tuple2<String,Boolean>> resolveCache(String lastCommand) {
    ...
    public static Optional<Tuple2<String,Boolean>> getExplicitWrite(String query) {
    ...
    private static String getGorDictSignature(GorSession gorPipeSession, String fileName) {
    ...
    public static String fileFingerPrint(String fileName, Map<String,String> singleFileSignatureMap, GorSession gorPipeSession) {
    ...
    public static String getFileSignatureAndUpdateSignatureMap(GorSession session, Map<String,String> fileSignatureMap, Map<String,String> singleFileSignatureMap, String commandToExecute, List<String> usedFiles) {
        var fileSignature = "";
        if (CommandParseUtilities.isDictionaryQuery(commandToExecute)) {
            ...
            fileSignature = fileSignatureMap.computeIfAbsent(fileListKey, (k) -> StringUtilities.createMD5(usedFiles.stream().map(x -> fileFingerPrint(x, singleFileSignatureMap, session)).collect(Collectors.joining(" ")) + signatureKey));
        }

        return fileSignature;
    }

    private static Map<String, ExecutionBlock> expandMacros(GorContext context, Map<String,String> aliases, Map<String,ExecutionBlock> creates, boolean valid) {
    ...
    private static Tuple2<Map<String,ExecutionBlock>,Map<String,ExecutionBlock>> splitBasedOnDependencies(VirtualFileManager virtualFileManager, Map<String,ExecutionBlock> executionBlocks) {
        ...
        for (Map.Entry<String,ExecutionBlock> executionBlock : executionBlocks.entrySet()) {
            var ovirt = virtualFileManager.get(executionBlock.getKey());
            if (ovirt.isDefined()) {
                var x = ovirt.get();
                if (x.fileName() == null) {
