...
class InflateColumn extends CommandInfo("INFLATECOLUMN",
  ...
  CommandOptions(gorCommand = true, norCommand = true))
{
  override def processArguments(context: GorContext, argString: String, iargs: Array[String], args: Array[String], executeNor: Boolean, forcedInputHeader: String): CommandParsingResult = {
    ...
    if (columnIndex < 2) {
      throw new GorParsingException("Invalid column for column inflate")
    }

    CommandParsingResult(InflateAnalysis(columnIndex), forcedInputHeader)
  }
}
