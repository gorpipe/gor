...
class UTestUntilAnalysis extends AnyFlatSpec {
  ...
  "process" should "pass single row through when condition is false" in {
    ...
    val pipe = UntilAnalysis(context, executeNor = false, "A=='CDE'", header) | sink
  ...
  it should "not pass single row through when condition is true" in {
    val sink = AnalysisSink()
    val header = "chrom\tpos\tA"
    val pipe = UntilAnalysis(context, executeNor = false, "A=='ABC'", header) | sink
    pipe.setRowHeader(RowHeader(header, List("S", "I", "S").toArray))

    val r = RowObj("chr1\t1\tABC")
    pipe.process(r)

    assert(sink.rows.isEmpty)
  }
}
