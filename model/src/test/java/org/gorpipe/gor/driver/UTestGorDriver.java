...
public class UTestGorDriver {
    ...
    public void testIndexedSourceReference() throws IOException {
        DataSource source = gorDriver.getDataSource(new IndexableSourceReference(DataUtil.toFile("../tests/data/gor/genes", DataType.GOR), "foo", "bar", null, null, 0, null));
    ...
    public void testIndexedSourceReferenceWithLinkFile() throws IOException {
        ...
        DataSource source = gorDriver.getDataSource(new IndexableSourceReference(linkFile.getAbsolutePath(), "foo", "bar", null, null, 0, null));
        Assert.assertTrue("source should exists", source.exists());
    ...
    public void testIndexedSourceReferenceWithLinkFileAndNoLinkExtension() throws IOException {
        ...
        // Create indexable source references
        DataSource source = gorDriver.getDataSource(new IndexableSourceReference(linkFile.getAbsolutePath().replace(DataType.LINK.suffix, ""), "foo", "bar", null, null, 0, null));
        Assert.assertTrue("source should exists", source.exists());
